            ...
            .addOnCompleteListener(this) { task ->
                runOnUiThread {
                    ...
                    if (task.isSuccessful) {
                        ...
                        if (exception is com.google.firebase.auth.FirebaseAuthUserCollisionException || exception?.message?.contains("The email address is already in use") == true) {
                            AlertDialog.Builder(this)
                                .setTitle("账号已注册")
                                .setMessage("该账号已注册，是否登录？")
                                .setPositiveButton("是") { _, _ ->
                                ...
                                .show()
                        } else if (exception?.message?.contains("network") == true) {
                            Toast.makeText(this, "网络异常，请重试", Toast.LENGTH_SHORT).show()
                        } else {
